...
void ListIEProcesses()
{
	...
	if (Process32First(hSnapshot, &pe32))
	{
		do
		{
			...
			//WCHAR wszExeName[0x100 + 1];
			//MultiByteToWideChar(CP_ACP, 0, pe32.szExeFile, strlen(pe32.szExeFile) + 1, wszExeName, sizeof(wszExeName) / sizeof(wszExeName[0]));
...
int ___tmainCRTStartup(int argc, WCHAR* argv[])
{
	if ((argc < 2) || (wcscmp(argv[1], L"-l") && (argc < 3)))
	{
		PrintHelp();
		return 1;
	}
	...
	if (hProcess)
	{
		size_t strSize = (wcslen(path) + 1) * sizeof(CHAR);
		...
		// LPVOID pLoadLibraryW = GetProcAddress(GetModuleHandle("kernel32"), "LoadLibraryA");
		LPVOID pLoadLibraryW = GetProcAddress(GetModuleHandle(L"kernel32"), "LoadLibraryA");

		if (pLoadLibraryW == nullptr)
		{
			printf("get loadlibraryA func error\n");
			return 0;
		}
